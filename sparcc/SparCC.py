...
def run_sparcc(f, **kwargs):
    ...
    for xi in range(xiter):
        ...
        Var_mat_temp[tuple(inds)]   = 0
        Var_mat_temp.T[tuple(inds)] = 0
        ...
        if len(excluded_comp_new)>0:
            #print "EXCLUDED COMP: ", excluded_comp
...
def driver():
    ...
    parser.add_option("-p", "--pval_file", dest="pval_file", type = 'str',
